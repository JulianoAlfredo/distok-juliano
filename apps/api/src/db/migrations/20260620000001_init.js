...
/**
 * Schema inicial do DISTOK (MySQL / InnoDB / utf8mb4).
 * DDL fiel a docs/architecture.md §3. Isolamento multi-tenant é garantido
 * na camada de aplicação (TenantScopedRepository), não por RLS.
 */
...
exports.down = async function down(knex) {
  ...
  await knex.raw('DROP TABLE IF EXISTS stock_balance');
  await knex.raw('DROP TABLE IF EXISTS stock_movements');
  await knex.raw('DROP TABLE IF EXISTS products');
  await knex.raw('DROP TABLE IF EXISTS tenant_terminology');
  await knex.raw('DROP TABLE IF EXISTS tenant_branding');
